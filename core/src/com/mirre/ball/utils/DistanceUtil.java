...
public class DistanceUtil {
	...
	public static boolean inSight(SimpleMovingObject target, SimpleMovingObject targeter, int maxXDistance, int maxYDistance, boolean facingCheck){
		...
		boolean right = (targeter.isRightOf(target.getBounds()) && facingCheck ? target.getDirection() == Direction.RIGHT : true);
		boolean left = (targeter.isLeftOf(target.getBounds()) && facingCheck ? target.getDirection() == Direction.LEFT : true);
